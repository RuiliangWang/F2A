...
! Rotor, Nacelle, Tower mass center
   ! CoG of rotor, tower and nacelle.
  
   
   CoGTwr(1) = CoGTwr(1) * AllOuts(YawBrTDxt) + CoGTwr(3)* sin(PtfmMotAQWA(5))
   CoGTwr(2) = CoGTwr(2) * AllOuts(YawBrTDyt) + CoGTwr(3)* sin(PtfmMotAQWA(4))
   CoGTwr(3) = CoGTwr(3) 
   CoGRot(1) = CoGTwr(1) + HubCM + OverHang*cos(ShftTilt/180.0*3.1415926) + FASTHH * sin(PtfmMotAQWA(5))
   CoGRot(2) = CoGTwr(2) + FASTHH * sin(PtfmMotAQWA(4))
   CoGRot(3) = FASTHH 
   CoGNac(1) = NacCMxn+CoGTwr(1) + (NacCMzn+TowerHt)*sin(PtfmMotAQWA(5))
   CoGNac(2) = NacCMyn+CoGTwr(2) + (NacCMzn+TowerHt)*sin(PtfmMotAQWA(4))
   CoGNac(3) = NacCMzn+TowerHt

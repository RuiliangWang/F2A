...
! Yang: Start of adding the use of TMD
USE                             TMD
! Yang: End of adding the use of TMD
...
! Yang: allocate the normal modal shape functions for earthquake force calculation
IF (.NOT. ALLOCATED( TwrFASF_Eq ) ) THEN
   ALLOCATE ( TwrFASF_Eq(2,TTopNode,0:2) , STAT=Sttus )
   IF ( Sttus /= 0 )  THEN
      CALL ProgAbort(' Error allocating memory for the TwrFASF array.')
   ENDIF
ENDIF

IF (.NOT. ALLOCATED( TwrSSSF_Eq ) ) THEN
   ALLOCATE ( TwrSSSF_Eq(2,TTopNode,0:2) , STAT=Sttus )
   IF ( Sttus /= 0 )  THEN
      CALL ProgAbort(' Error allocating memory for the TwrSSSF_Eq array.')
   ENDIF
ENDIF
! Yang: end of allocating
...
! Yang: Start of allocating the TMD variables
...
! Yang: End of allocating the TMD variables
...
! Yang: Start of allocating the TMD variables. More specific, the partial velocity, forces, and moments of the TMDs
...
ENDIF
! Yang: Allocating the partial forces and moments
...
ENDIF
! Yang: End of adding the allocations
...
USE                             TMD
! Yang: End of adding the use of TMD
! Yang: use the AQWAinFAST for giving values to PtfmMomAQWA and PtfmFrcAQWA
USE                             AQWAinFAST
!end of change
...
! Yang: Start of adding the definations of linear acceleration output variables for the TMDs
...
! Yang: End of the proposed change.
...
! Yang: Start of initializing the linear accelerations of the TMDS
...
! Yang: Start of adding the statements for the location of the TMDs
!       The dotproducts depend on their locations.
...
! Yang: Tower base loads. Convert the unit from kN to N, with repect to the platform reference point frame
PtfmFrcAQWA(1) = AllOuts( TwrBsFxt) *1000.0 
PtfmFrcAQWA(2) = AllOuts( TwrBsFyt) *1000.0  
PtfmFrcAQWA(3) = AllOuts( TwrBsFzt) *1000.0 
PtfmMomAQWA(1) = AllOuts( TwrBsMxt) *1000.0 
PtfmMomAQWA(2) = AllOuts( TwrBsMyt) *1000.0  
!PtfmMomAQWA(3) = AllOuts( TwrBsMzt) *1000.0 
PtfmMomAQWA(3) = DOT_PRODUCT( MomX0Trb - MomBNcRt, a2 ) *1000.0  ! platform local yaw moment (minus the tower-top yaw momement)
PtfmMomAQWA_Yaw = ( MomX0Trb(2) - MomBNcRt(2) ) *1000.0
...
! Yang: Change for tower cog
REAL (Reki)                  :: TempTwrMass                                     ! Temp tower mass for the calculation of tower CoG
...
! Yang: Tower mass center
   TempTwrMass  = TempTwrMass + ElmntMass * HNodesNorm(J)
...
DO J = 1,TwrNodes    ! Loop through the tower nodes / elements
   ...
   DO I = 1,2     ! Loop through all tower DOFs in one direction
      ...
      MTFA_eq(I,I) = MTFA_eq(I,I)+ElmntMass*TwrFASF(I,J,0)
      MTSS  (I,I) = MTSS  (I,I) + ElmntMass*TwrSSSF(I,J,0)*TwrSSSF(I,J,0)
      MTSS_eq(I,I) = MTSS_eq(I,I)+ElmntMass*TwrSSSF(I,J,0)
...
ENDDO ! J - Tower nodes / elements
! Yang: Normalizing the normal shape functions(only the second modal shapes)
TwrFASF_Eq(2,:,0) = TwrFASF(2,:,0)/MaxVal(abs(TwrFASF(2,:,0)))
TwrSSSF_Eq(2,:,0) = TwrSSSF(2,:,0)/MaxVal(abs(TwrSSSF(2,:,0)))
! Obtain madal participation factors
ModalPartFct(1) = MTFA_eq(1,1)/(MTFA(1,1)-TwrTpMass)
ModalPartFct(2) = MTFA_eq(2,2)/(MTFA(2,2)-TwrTpMass)
ModalPartFct(3) = MTSS_eq(1,1)/(MTSS(1,1)-TwrTpMass)
ModalPartFct(4) = MTSS_eq(2,2)/(MTSS(2,2)-TwrTpMass)
! Yang: End of normalizing
...
! Yang: Calculate the tower CoG   
   CoGTwr(3) = TwrFlexL * (TempTwrMass/TwrMass) + TwrRBHt
   CoGTwr(1) = SHP( TempTwrMass/TwrMass, 1.0, TwFAM1Sh(:), 0 ) ! Normalized fraction of CoG_x offx
   CoGTwr(2) = SHP( TempTwrMass/TwrMass, 1.0, TwSSM1Sh(:), 0 ) ! Normalized fraction of CoG_y offy
! Calulate tower inertia
do J=1,TwrNodes
   ElmntMass    = MassT(J)*DHNodes(J)
   TwrIner(1) = TwrIner(1) + ElmntMass * (HNodes(J)-CoGTwr(3))**2
   TwrIner(2) = TwrIner(1) 
   TwrIner(3) = TwrIner(3) + DHNodes(J)*(InerTFA(J)+InerTSS(J))*0.5
enddo
! End of the change
...
! Yang: Start of adding a variable for tower feedback control.
REAL(ReKi)                   :: TwrAccelSS                                      ! side-side Tower Acceleration.  Used for tower feedback control.
! Yang: End of the proposed change.
INTEGER(4)                   :: I                                               ! Generic index
INTEGER(4)                   :: K                                               ! Loops through blades.
...
! Yang: Start of adding the active control for the TMDs in simulink
...
TwrAccel = DOT_PRODUCT( LinAccEO, b1 )
TwrAccelSS = DOT_PRODUCT( LinAccEO, b3 )
...
IF (FAST2AQWA .or.( DOF_Flag(DOF_Sg) .and. DOF_Flag(DOF_Sw) .and. DOF_Flag(DOF_Hv) .and. DOF_Flag(DOF_R) .and. DOF_Flag(DOF_P) .and. DOF_Flag(DOF_Y)) ) THEN
   CALL Gauss( AugMat( SrtPS(7:NActvDOF), SrtPSNAUG(7:(NActvDOF+1)) ), NActvDOF-6, SolnVec )
   QD2T = 0.0
   DO I = 7,NActvDOF ! Loop through all active (enabled) DOFs
      QD2T(SrtPS(I)) = SolnVec(I-6)
   ENDDO             ! I - All active (enabled) DOFs
   !QD2T(DOF_Sg ) = PtfmAcceAQWA(1)
   !QD2T(DOF_Sw ) = PtfmAcceAQWA(2)
   !QD2T(DOF_Hv ) = PtfmAcceAQWA(3)
   !QD2T(DOF_R  ) = PtfmAcceAQWA(4)
   !QD2T(DOF_P  ) = PtfmAcceAQWA(5)
   !QD2T(DOF_Y) = PtfmAcceAQWA(6)
ELSE
...
DO K = 1,NumBl ! Loop through all blades
! Yang: Start of changing the index of the DOFs due to consideration of TMDs
...
!   DOF_BF(K,2) = 18 + 3*(K-1)       ! 2nd blade flap mode--DOFs 18, 21, and 24 for blade 1, 2, and 3, respectively
   DOF_BF(K,1) = 18 + 3*(K-1)       ! 1st blade flap mode--DOFs 18, 21, and 24 for blade 1, 2, and 3, respectively
   DOF_BE(K,1) = 19 + 3*(K-1)       ! 1st blade edge mode--DOFs 19, 22, and 25 for blade 1, 2, and 3, respectively
   DOF_BF(K,2) = 20 + 3*(K-1)       ! 2nd blade flap mode--DOFs 20, 23, and 26 for blade 1, 2, and 3, respectively
ENDDO          ! K - All blades
...
! Yang: Start of adding the calculation of TMD motions
...
! Yang: End of adding the calculation of TMD motions
...
USE                             Constants
! Yang: Start of the change of using Seismic Module
USE                             Seismic
! Yang: End of the change of using Seismic Module
! Yang: Start of adding the use of TMD
USE                             TMD
! Yang: End of adding the use of TMD
USE                             AQWAinFAST
! Yang: Change for the use of AQWAinFAST module
...
! Yang: Start of adding the variables for the TMDs EOM 
!nacelle
...
! Yang: Start of the change of defining variables for seismic analysis
REAL(Reki)                   :: EqAcce   (3)                                    ! Earthquake acceleration vector
INTEGER(4)                   :: iTimeStep                                               ! Loops through some or all of the DOFs.

! Yang: end of the change of defining variables for seismic analysis
...
! Yang: Start of adding the earthquake acceleration calculation
iTimeStep = ZTime/DT+1
IF (SeismicLdMode) THEN
   EqAcce = PtfmAccel(1:3,iTimeStep)
ELSE 
   EqAcce = 0.0
ENDIF
! Yang: End of the proposed change.
!
! Yang: Start of initializing the TMD linear acceleration variables
LinAccETmdXt  = 0.0
LinAccETmdYt  = 0.0
! Yang: End of initializing the TMD linear acceleration variables
...
! Yang: Start of changing the coordinates for the TMDs
...
! Yang: Start of calculate the cross product of angular velocity and position of the TMDs
...
ENDSELECT
! Yang: SEnd of the proposed change.
...
! Yang: Start of finding the linear acceleration and velocity terms of the TMDs
SELECT CASE ( TmdXLoc ) ! Which tmdx location?
...
! Yang: Add the earthquake force to rotor
...
ENDIF
! Yang: Add the earthquake force to RFrl
...
! Yang: Add the earthquake force to Boom and Tfin
TmpVec1 = -BoomMass*( Gravity*z2 + LinAccEIt)! + EqAcce )  ! The portion of FrcWTailt associated with the BoomMass
TmpVec2 = -TFinMass*( Gravity*z2 + LinAccEJt)! + EqAcce )  ! The portion of FrcWTailt associated with the TFinMass
...
! Yang: Start of adding partial force and moment terms of the TMDs
SELECT CASE ( TmdXLoc ) ! Which tmdx location?
	CASE ( 1 )              ! nacelle
...
! Yang : Add the earthquake force to nacelle
TmpVec1 = -NacMass*( Gravity*z2 + LinAccEUt  ) !- TwrTpMass*EqAcce     ! The portion of FrcONcRtt associated with the NacMass
...
! Yang: Start of calculating the force and moment of the TMDs
SELECT CASE ( TmdXLoc ) ! Which tmdx location?
	CASE ( 1 )              ! nacelle
...
ENDSELECT
! Yang: END of calculating the force and moment of the TMDs
...
! Yang: Start of calculating the partial forces and moments of the TMDs
SELECT CASE ( TmdXLoc ) ! Which tmdx location?
	CASE ( 2 )              ! platform
     ! initialize partial forces to 0.0
     PFrcZTmdX  = 0.0
...
! Yang: Start of calculating the forces and moments of the TMDs
SELECT CASE ( TmdXLoc ) ! Which tmdx location?
	CASE ( 2 )              ! platform
...
! Yang: Start of calculating the stopping force
...
! Yang: End of calculating the stopping force.
...
! Yang: Start of calculating the augmented matrix of the QD2T associated with the TMDs
SELECT CASE ( TmdXLoc ) ! Which tmdx location?
...
! Yang: End of calculating the augmented matrix of the QD2T associated with the TMDs
...
! Yang: Start of adding the orientation angles variables for coordinates calcualtion
...
! Yang: End of adding the orientation angles variables for coordinates calcualtion
...
! Yang: Start of calculating the TMD system coordinates
	...
	! TMDX platform coordinate system:
	...
	! TMDY platform coordinate system:

	tmdpy1 = CTmdYAngle*a1 - STmdYAngle*a3     
	tmdpy2 = a2                         
	tmdpy3 = STmdYAngle*a1 + CTmdYAngle*a3     

! Yang: End of calculating the TMD system coordinates
...
! Yang: Start of the proposed change to include counter variable for the TMDs
NPTmdXE  = 0
NPTmdYE  = 0
! Yang: End of proposed change. 
...
! Yang: Start of the proposed change to include TMD counters and pointers
   NPTmdXE     = NPTmdXE     + 1
   NPTmdYE     = NPTmdYE     + 1
   
   PTmdXE    (  NPTmdXE    ) = DOF_Sg
   PTmdYE    (  NPTmdYE    ) = DOF_Sg
! Yang: End of the proposed change to include TMD counters and pointers
...
! Yang: Start of adding the TmdX and TmdY DOF
...
! End: Start of adding the TmdX and TmdY DOF
...
IF ( Step < 3 )  THEN   ! Use Runge-Kutta integration at the the start of the simulation (first 3 steps).
   ...
   QT  = Q (:,IC(1))
   QDT = QD(:,IC(1))
! Yang: Force the platform motion and velocity to be same as the input from AQWA
   QT(DOF_Sg)   = PtfmMotAQWA(1)     
   QT(DOF_Sw)   = PtfmMotAQWA(2)     
   QT(DOF_Hv)   = PtfmMotAQWA(3)     
   QT(DOF_R)    = PtfmMotAQWA(4)     
   QT(DOF_P)    = PtfmMotAQWA(5)     
   QT(DOF_Y)    = PtfmMotAQWA(6)     
   QDT(DOF_Sg)  = PtfmVelAQWA (1) 
   QDT(DOF_Sw)  = PtfmVelAQWA (2) 
   QDT(DOF_Hv)  = PtfmVelAQWA (3) 
   QDT(DOF_R)   = PtfmVelAQWA (4) 
   QDT(DOF_P)   = PtfmVelAQWA (5) 
   QDT(DOF_Y)   = PtfmVelAQWA (6) 
! End of the change

   CALL RtHS
! Yang: Force the platform acceleration to be 0 
   QD2T (DOF_Sg) = PtfmAcceAQWA(1)
   QD2T (DOF_Sw) = PtfmAcceAQWA(2)
   QD2T (DOF_Hv) = PtfmAcceAQWA(3)
   QD2T (DOF_R)  = PtfmAcceAQWA(4)
...
! End of the change
   ...
   ENDDO          ! I - All DOFs
! Yang: Force the platform motion and velocity to be same as the input from AQWA
   !Q(DOF_Sg,IC(NMX))   = PtfmMotAQWA(1)     
   !Q(DOF_Sw,IC(NMX))   = PtfmMotAQWA(2)     
   !Q(DOF_Hv,IC(NMX))   = PtfmMotAQWA(3)     
   !Q(DOF_R,IC(NMX))    = PtfmMotAQWA(4)     
   !Q(DOF_P,IC(NMX))    = PtfmMotAQWA(5)     
   !Q(DOF_Y,IC(NMX))  = PtfmMotAQWA(6)     
   !QD(DOF_Sg,IC(NMX))  = PtfmVelAQWA (1) 
   !QD(DOF_Sw,IC(NMX))  = PtfmVelAQWA (2) 
   !QD(DOF_Hv,IC(NMX))  = PtfmVelAQWA (3) 
   !QD(DOF_R,IC(NMX))   = PtfmVelAQWA (4) 
   !QD(DOF_P,IC(NMX))   = PtfmVelAQWA (5) 
   !QD(DOF_Y,IC(NMX)) = PtfmVelAQWA (6) 
! End of the change
...
! End of change
   QD2(:,IC(NMX)) = QD2T
! Yang: Force the platform acceleration to be 0 
   !QD2(DOF_Sg,IC(NMX))  = PtfmAcceAQWA(1)
   !QD2(DOF_Sw ,IC(NMX)) = PtfmAcceAQWA(2)
   !QD2(DOF_Hv ,IC(NMX)) = PtfmAcceAQWA(3)
   !QD2(DOF_R  ,IC(NMX)) = PtfmAcceAQWA(4)
   !QD2(DOF_P  ,IC(NMX)) = PtfmAcceAQWA(5)
   !QD2(DOF_Y,IC(NMX)) = PtfmAcceAQWA(6)
! End of the change
...
!=======================================================================
SUBROUTINE TimeMarch (iSts,iStps)
...
IMPLICIT                        NONE
   ...
   ! Local variables.
! Yang
integer :: iSts                                                       ! status of calling FAST. 0 = first,1 = the rest, 2 = the end.
integer :: i
integer :: iStps                                                      ! Number of steps for running the loop of time marching process
!REAL (Reki)           :: PtfmMotIn(6),PtfmVelIn(6)                    ! Platfrom motion and velocity in the six DoFs
! Yang: end of change
...
! Yang: first time to call FAST
IF (iSts ==0 .and. PrtFASTRslts) THEN
   CALL WrOutHdr
...
DO i=1,iStps
   ...
   ! Advance time:
   !IF (PrtFASTRslts) THEN  ! not update time when STAGE=2 (prediction state in AQWA) 
      Step  = Step + 1
      ZTime = Step*DT
   !ENDIF
   ...
   IF ( ZTime >= TStart )  THEN
      ...
      IF ( MOD( Step, DecFact ) == 0  )  CALL WrOutput  ! not output results when STAGE=2 (prediction state in AQWA) 
   ENDIF
...
INTEGER(4)                   :: I                                               ! Loops through all platform DOFs.
INTEGER(4)                   :: J                                               ! Loops through all platform DOFs.
! Yang: Start of defining the variables
INTEGER(4)                   :: iTimeStep                                       ! Current time step used to obtain the current eq acceleration.
REAL(ReKi)                   :: ElmntMass                                       ! Tower node mass.
REAL(ReKi)                   :: ModalMass_FA                                    ! Tower node modal mass in fore-aft direction.
REAL(ReKi)                   :: ModalMass_SS                                    ! Tower node modal mass in side-side direction.

! Yang: End of defining the variables
...
! Yang: Start of adding a case of tower loading that is Morison's equations and user-defined tower loading model are both used.
      CASE ( 3 )  ! Both Morison's equation and user-defined from routine UserTwrLd are used.
...
! CALL the user-defined tower loading model:
   ...
   ENDSELECT ! end select of tower loading case.
! Yang: End of adding a case of tower loading
...
! Yang: Start of adding earthquake forces
iTimeStep = ZTime/DT+1
ElmntMass = MassT(JNode) !*DHNodes(JNode) 
IF (SeismicLdMode) THEN
   ModalMass_FA = MassT(JNode)*(TwrFASF(1,JNode,0)+TwrFASF(2,JNode,0))
   !ModalMass_SS = ElmntMass*(TwrSSSF(1,JNode,0))!+TwrSSSF_Eq(2,JNode,0))
   !ModalMass_FA = ElmntMass*(abs(TwrFASF(1,JNode,0))+abs(TwrFASF_Eq(2,JNode,0)))
   ModalMass_SS = MassT(JNode)*(TwrSSSF(1,JNode,0)+TwrSSSF(2,JNode,0))
   TwrFt(1) = TwrFt(1) - ModalMass_FA*PtfmAccel(1,iTimeStep)
   TwrFt(2) = TwrFt(2) - ModalMass_SS*PtfmAccel(2,iTimeStep)
ENDIF
!if ((ZTime>EqDelay) .and. (JNode==50))then
!WRITE(*,*) 'Info from twrlding subroutine'
!write (*,*) 'ZTime = ',ZTime
!write (*,*) 'ElmntMass = ',ElmntMass
!write (*,*) 'TwrFt(1) = ',TwrFt(1)
!write (*,*) 'TwrFt(1)NoEq = ',TwrFt(1) + ModalMass_FA*PtfmAccel(1,iTimeStep)
!write (*,*) 'PtfmAccel = ',PtfmAccel(1,iTimeStep)
!write (*,*) 'ModalMass_FA = ',ModalMass_FA
!write (*,*) 'ModalMass_SS = ',ModalMass_SS
!write (*,*) 'FAEq_F = ',ModalMass_FA*PtfmAccel(1,iTimeStep)
!write (*,*) 'SSEq_F = ',ModalMass_SS*PtfmAccel(2,iTimeStep)
!endif
! Yang: End of adding earthquake forces
RETURN
END SUBROUTINE TwrLoading
!=======================================================================
!  : Start of adding the subroutines to calculate the stop force of the TMDs
!=======================================================================

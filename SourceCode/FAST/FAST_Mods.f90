...
! Yang: begin of adding the defination of seismic module in primary file of FAST
CHARACTER(1024)             ,SAVE :: SeismicFile                                     ! The name of the seismic configuration file.
LOGICAL                     ,SAVE :: SeismicLdMode                                   ! Consider earthquake or not.
! Yang: end of adding the defination of seismic module in primary file of FAST
...
END MODULE TurbCont
!=======================================================================
! Yang: add a module that contains variables of SSI parameters
MODULE SSI


   ! This MODULE stores input variables for SSI configuration.


USE                             Precision

INTEGER(4)                  ,SAVE :: SSIMode                                        ! Model for SSI (0: none, 1: Coupled Springs (Wolf method), 2: Coupled Springs (Specified stiffness and damping),3: Distributed Springs)
REAL(ReKi)                  ,SAVE :: ShrModuSoil                                    ! The shear modulus of local soil (Unit:MPa) [Only used when SSIMode = 1]
REAL(ReKi)                  ,SAVE :: DenSoil                                        ! The denisty of local soil (Unit: kg/m^3) [Only used when SSIMode = 1]
REAL(ReKi)                  ,SAVE :: PoissRatio                                     ! The poisson ratio of local soil (Unit: -) [Only used when SSIMode = 1]
REAL(ReKi)                  ,SAVE :: RadiusPtfm                                     ! The radius of platform (Unit: m) [Only used when SSIMode = 1]
CHARACTER(1024)             ,SAVE :: InFileGKCM                                     ! Input file for specified stiffness and damping matrices mode. The file contains the stiffness and damping and added-mass matrices. [Only used when SSIMode = 2]
CHARACTER(1024)             ,SAVE :: InFileDS	                                   ! Input file for DS model whicn defines the stiffness distribution for a pile [Only used when SSIMode = 3]
CHARACTER(1024)             ,SAVE :: InFileNonlinear                                ! Input file for DS model whicn defines the stiffness distribution for a pile [Only used when SSIMode = 3]

! Yang: begin of adding the units for the open of SSI files 
INTEGER(4)                  ,SAVE :: UnGKC     = 30                                  ! I/O unit number for the specified stiffness and damping matrices file.
INTEGER(4)                  ,SAVE :: UnLDS     = 31                                  ! I/O unit number for the linear DS model input file.

! Yang: end of adding the units for the open of SSI files

END MODULE SSI

! Yang: add a module that contains variables of nonlinear SSI parameters
MODULE NonLinearSSI

    INTEGER(4),SAVE :: NSets 
    INTEGER(4),SAVE :: NumStff = 20                         ! Number of the interpolation points for the stiffness for each length
    REAL(4),allocatable,SAVE :: Depth (:)                   ! Used to store the depths which have nonlinear springs
    REAL(4),allocatable,SAVE :: yDeflec(:,:)
    REAL(4),allocatable,SAVE :: pForce(:,:)
    INTEGER(4),SAVE :: UnNLDS    = 32                                  ! I/O unit number for the nonlinear DS model input file (p-y curves).
END MODULE NonLinearSSI
! Yang: add a module that is used for any need of interpolation using cubic method. The module will be used for nonlinear soil stiffness calculation.
MODULE CubicInterp
! This module is developed by Y.Yang for interpolation using cubic method.
! Date: 13, July 2018
contains
! Cubic interpolation
SUBROUTINE CSIEZ(n,x,y,m,xx,yy)
! n: Number of input points
! x,y: coordinates of input points (vectors)
! m: Number of output points
! xx,yy: coordinates of output points (vectors)

  INTEGER(4):: n,m
  REAL(4)  :: x(n),y(n),y2(n),xx(m),yy(m)
  
  CALL SPLINE(n,x,y,y2)
  
  DO i=1,m
    CALL SPLINT(n,x,y,y2,m,xx(i),yy(i))
  ENDDO

END SUBROUTINE CSIEZ
...
! n: Number of input points
! x,y: coordinates of input points (vectors)
! y2: 2nd deriavtive associated with the output points
...
IMPLICIT NONE
! n: Number of input points
! xa, ya: coordinates of input points (vectors)
! y2a: 2nd deriavtive obtained from the subroutine spline
! m: number of interpolation points 
! x,y : coordinates of the interpolated point

integer , parameter :: NP = SELECTED_REAL_KIND(8)
INTEGER(4)::  n,m
REAL(4)::  aaa,xa(n),ya(n),y2a(n),x,y
INTEGER(4)::  k,khi,klo
REAL(4)::  a,b,h
...
REAL (Reki)          ,SAVE :: CoGRot(3)                         ! Rotor CoG
REAL (Reki)          ,SAVE :: CoGNac(3)                         ! Nacelle CoG
REAL (Reki)          ,SAVE :: CoGTwr(3)                         ! Tower CoG
REAL (Reki)          ,SAVE :: TwrIner(3) = 0.0                        ! Tower inertia xx, yy, zz

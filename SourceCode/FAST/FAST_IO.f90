...
USE                             TMD
! Yang: End of adding the use of TM
...
! Yang: add the out check for the kinematic variables of the TMDs 
   CHARACTER( 9),PARAMETER  :: ValidParamAry(1122) =  (/ &                         ! This lists the names of the allowed parameters, which must be sorted alphabetically
                               ...
                               "TIPSPDRAT","TMDXAXN  ","TMDXAXT  ","TMDXDXN  ","TMDXDXT  ","TMDXVXN  ","TMDXVXT  ", & ! Yang: add the out check for the kinematic variables of the TMDs 
                               "TMDYAYN  ","TMDYAYT  ","TMDYDYN  ","TMDYDYT  ", "TMDYVYN  ","TMDYVYT  ",&             ! Yang: add the out check for the kinematic variables of the TMDs 
                               "TOTWINDV ","TSR      ","TTDSPAX  ","TTDSPFA  ","TTDSPPTCH","TTDSPROLL", &
   ...
   INTEGER,      PARAMETER  :: ParamIndxAry(1122) =  (/ &                          ! This lists the index into AllOuts(:) of the allowed parameters ValidParamAry(:)
                                 Anch1Ang ,  Anch1Ten ,  Anch2Ang ,  Anch2Ten ,  Anch3Ang ,  Anch3Ten ,  Anch4Ang , &
                                 Anch4Ten ,  Anch5Ang ,  Anch5Ten ,  Anch6Ang ,  Anch6Ten ,  Anch7Ang ,  Anch7Ten , &
                                ...
                                TipSpdRat ,  TmdXAxn  ,  TmdXAxt  ,  TmdXDxn  ,  TmdXDxt  ,  TmdXVxn  ,  TmdXVxt  , & ! Yang: add the out check for the kinematic variables of the TMDs 
                                 TmdYAyn  ,  TmdYAyt  ,  TmdYDyn  ,  TmdYDyt  ,  TmdYVyn  ,  TmdYVyt  , &             ! Yang: add the out check for the kinematic variables of the TMDs 
                                 TotWindV , TipSpdRat , YawBrTDzt , YawBrTDxt , YawBrRDyt , YawBrRDxt , &
   ...
   CHARACTER(10),PARAMETER  :: ParamUnitsAry(1122) =  (/ &                         ! This lists the units corresponding to the allowed parameters
                               ...
                               "(deg)     ","(deg)     ","(deg)     ","(deg)     ","(deg)     ","(deg)     ","(deg)     ", &
                               "(-)       ","(m/s^2)   ","(m)       ","(m/s)     ","(m/s^2)   ","(m)       ","(m/s)     ", & ! Yang: add the out check for the kinematic variables of the TMDs 
                               "(m/s^2)   ","(m)       ","(m/s)     ","(m/s^2)   ","(m)       ","(m/s)     ", &              ! Yang: add the out check for the kinematic variables of the TMDs 
                               "(m/s)     ","(-)       ","(m)       ","(m)       ","(deg)     ","(deg)     ", &
...
!BJJ: THE FOLLOWING USED TO BE PART OF THE SCRIPT, BUT I REWROTE IT TO BE MORE EFFICIENT
   ...
   END IF
! Yang: Add the statements to check the validities of TMD output channels
   IF (.NOT. TmdXDOF) THEN
      InvalidOutput(  TmdXAxt) = .TRUE.
      InvalidOutput(  TmdXDxt) = .TRUE.
      InvalidOutput(  TmdXVxt) = .TRUE.
      InvalidOutput(  TmdXAxn) = .TRUE.
      InvalidOutput(  TmdXDxn) = .TRUE.
      InvalidOutput(  TmdXVxn) = .TRUE.
   ENDIF
   IF (.NOT. TmdYDOF) THEN
      InvalidOutput(  TmdYAyt) = .TRUE.
      InvalidOutput(  TmdYDyt) = .TRUE.
      InvalidOutput(  TmdYVyt) = .TRUE.
      InvalidOutput(  TmdYAyn) = .TRUE.
      InvalidOutput(  TmdYDyn) = .TRUE.
      InvalidOutput(  TmdYVyn) = .TRUE.
   ENDIF  
...
USE                             TurbConf
USE                             TurbCont
! Yang: Use the SSI module
USE                             SSI
! Yang Use the TMD module
USE                             TMD
! Yang: USE the BladedDLL module to replace the DISCON DLL name. 10-March-2020
...
!Yang: Not write the string when coupling with AQWA
...
! Yang: Start of reading seismic parameters
! - skip the comment line.
   CALL ReadCom ( UnIn, PriFile, 'Seismic loading' )
   CALL ReadLVar ( UnIn, PriFile, SeismicLdMode, 'SeismicLdMode', 'The flag of seismic loads calculation' )
   CALL ReadCVar ( UnIn, PriFile, SeismicFile, 'SeismicFile', 'Name of file containing seismic configuration' )
   IF ( PathIsRelative( SeismicFile ) ) SeismicFile = TRIM(PriPath)//TRIM(SeismicFile)
! Yang: End of reading seismic parameters
    
! Yang: Start of reading SSI parameter
    CALL ReadCom ( UnIn, PriFile, 'Soil-Structure Interaction' )
    CALL ReadIVar ( UnIn, PriFile, SSIMode, 'SSIMode', 'Model for SSI ' )
    CALL ReadRVar ( UnIn, PriFile, ShrModuSoil, 'ShrModuSoil', 'The shear modulus of local soil' )
    CALL ReadRVar ( UnIn, PriFile, DenSoil, 'DenSoil', 'The denisty of local soil' )
    CALL ReadRVar ( UnIn, PriFile, PoissRatio, 'PoissRatio', 'The poisson ratio of local soil' )
    CALL ReadRVar ( UnIn, PriFile, RadiusPtfm, 'RadiusPtfm', 'The radius of platform' )
    CALL ReadCVar ( UnIn, PriFile, InFileGKCM, 'InFileGKCM', 'Input file for specified stiffness and damping matrices mode.' )
    CALL ReadCVar ( UnIn, PriFile, InFileDS, 'InFileDS', 'Input file for DS model whicn defines the stiffness distribution for a pile' )
    CALL ReadCVar ( UnIn, PriFile, InFileNonlinear, 'InFileNonlinear', 'Input file for nonlinear DS model whicn defines the stiffness distribution for a pile' )
! Yang: End of reading SSI parameter
! Yang: Start of reading the Structural control module 14-Aug-2018
    CALL ReadCom ( UnIn, PriFile, 'Structural Control' )
    CALL ReadLVar ( UnIn, PriFile, StrcCtrlMode, 'StrcCtrlMode', 'The flag of structural control' )
    CALL ReadCVar ( UnIn, PriFile, StrcCtrlFile, 'StrcCtrlFile', 'Name of the file containing TMD control configurations' )
! Yang: End of reading the Structural control module
! Yang: Start of reading the Bladed Style DLL
    CALL ReadCom ( UnIn, PriFile, 'Bladed style DLL' )
    CALL ReadCVar ( UnIn, PriFile, DLL_FileName, 'DLL_FileName', 'DLL file name including the extension' )
    CALL ReadCVar ( UnIn, PriFile, DLL_ProcName, 'DLL_ProcName', 'Procedue name of the DLL control subroutine that will be called in the DLL' )
    CALL ReadCVar ( UnIn, PriFile, DLL_InFile, 'DLL_InFile', 'Input file name of the DLL in which the control parameters are defined' )
! Yang: End of the change. 10-march-2020    
!  -------------- OUTPUT PARAMETERS --------------------------------------------
...
! Yang: Start of adding the judgements for the conflicted settings of platform properties
USE                             SSI, ONLY: SSIMode
! Yang: End of adding the judgements for the conflicted settings of platform properties
...
! Yang: Start of adding judgements for aborting the program due to conflicted settings of platform mass matrix
IF ((SSIMode==2)) THEN ! DS model, only UserTwrLd will be called
    IF ((PtfmMass > 0.0) .OR. (PtfmRIner > 0.0) .OR. (PtfmPIner > 0.0) .OR. (PtfmYIner > 0.0) ) THEN
       CALL ProgAbort('The values of platform properties have to be zero.')
    ENDIF
ENDIF
! End of adding judgements for aborting the program due to conflicted settings of platform mass matrix
!  -------------- PLATFORM (CONT) ----------------------------------------------
...
! Yang: Start of adding a valid value of TwrLdMod: 3
         IF ( ( TwrLdMod < 0 ) .OR. ( TwrLdMod >= 4 )  )  THEN
            CALL ProgAbort ( ' TwrLdMod must be 0, 1, 2 and 3.' )
         END IF
! Yang: end of adding a valid value of TwrLdMod: 3

         IF (( TwrLdMod == 1 ) .or. ( TwrLdMod == 3 ))  THEN ! .TRUE if we will be using the built-in Morison's equation.
...
IF ( Echo )  THEN
   ...
   IF ( ( ADAMSPrep == 1 ) .OR. ( ADAMSPrep == 3 ) )  THEN  ! An ADAMS model will be created; thus, read in all the cols.
...
! Yang: Start of adding the use of Seismic module.
USE                             Seismic
USE                             SSI
! Yang: End of adding the use of Seismic module.
! Yang: Start of adding the use of TMD module
USE                             TMD
! Yang: END of adding the use of TMD module
IMPLICIT                        NONE
...
! Yang: Start of adding the judgement of initializing the seismic analysis.
! Open the seismic configuration file
IF (SeismicLdMode) THEN ! Calculate the earthquake loads
   CALL WrScr1 ( ' Using Seismic file: '//trim(SeismicFile))
   CALL ReadSeismicFile
ENDIF
! Yang: End of adding the judgement of initializing the seismic analysis.

! Yang: Start of adding the statement of reading the TMD control file. 14-Aug-2018
IF (StrcCtrlMode) THEN ! Read the TMD control input file
   CALL WrScr1 ( ' Using TMD control file: '//trim(StrcCtrlFile))
   CALL ReadTMDFile
ENDIF
! Yang: End of adding the statement of reading the TMD control file.
...
ENDIF
! Yang: Start of adding judgements for aborting the program due to conflicted settings
IF ((SSIMode==1) .OR. (SSIMode==2)) THEN   ! CS model, only UserPtfmLd will be called
   IF ((TwrLdMod == 2) .OR. (TwrLdMod == 3)) THEN
       CALL ProgAbort('user-defined tower loading routine only can be called for the case of SSIMode = 3.'// &
                      ' Please modify the value of SSIMode or the value of TwrLdMod in '//trim(PtfmFile))
   ELSEIF (( CompHydro) .AND. (TwrDraft > WtrDpth)) THEN
       CALL ProgAbort('TwrDraft cannot be greater than WtrDpth for the cases of SSIMode /= 0')
   ENDIF
ENDIF
IF (SSIMode==3 .or. SSIMode ==4 ) THEN ! DS model, only UserTwrLd will be called
   IF ((PtfmSgDOF) .OR. (PtfmSwDOF) .OR. (PtfmHvDOF) .OR. (PtfmRDOF) .OR. (PtfmPDOF) .OR. (PtfmYDOF) ) THEN
       CALL ProgAbort('The DOFs of platform cannot be enabled if a DS model has been applied.')
   ELSEIF ((PtfmLdMod == 1)) THEN
       CALL ProgAbort('The UserPtfmLd cannot be called if a DS model has been applied. Please change the value of PtfmLdMod.')
   ELSEIF ((TwrDraft <= WtrDpth)) THEN 
       CALL ProgAbort('TwrDraft should be greater than WtrDpth if a DS model has been applied.')
   ENDIF
   CALL WrScr1 ( 'NOTE: Tower file should contain structural properties of the draft embeded in the sea bed if a DS model has been applied.')
ENDIF
IF (SSIMode==0) THEN ! No soil effect
   IF ((PtfmSgDOF) .OR. (PtfmSwDOF) .OR. (PtfmHvDOF) .OR. (PtfmRDOF) .OR. (PtfmPDOF) .OR. (PtfmYDOF) ) THEN
       CALL ProgAbort('The DOFs of platform should not be enabled if SSI effect has not been considered.')
   ELSEIF ((PtfmLdMod == 1)) THEN
       CALL ProgAbort('The UserPtfmLd should not be called for a fixed base model. Please change the value of PtfmLdMod.')
   ENDIF
ENDIF
! Yang: End of adding judgements for aborting the program due to conflicted settings
...
! Yang: Start of the change of the number of DOFs   
IF ( NumBl == 2 )  THEN
   ! NDOF = 22
   NDOF = 24
ELSE
   ...
   NDOF = 26
ENDIF
! Yang: end of the change of the number of DOFs   
NAug = NDOF + 1
...
! Yang: Start of initializing the TMD parameters
TmdXDspInit = TmdXDsp
TmdXSprInit = TmdXSpr
TmdXDampInit = TmdXDamp

TmdYDspInit = TmdYDsp
TmdYSprInit = TmdYSpr
TmdYDampInit = TmdYDamp

TmdXNeut = TmdXRefxnt
TmdYNeut = TmdYRefynt

TmdXFextInit = TmdXFext
TmdYFextInit = TmdYFext
! Yang: End of initializing the TMD parameters
...
! Yang: Start of adding the output summary of TMD configuration.
IF ( TmdXDOF )  THEN
   WRITE (UnSu,FmtTxt)  ' Enabled    TmdX DOF.'
ELSE
   WRITE (UnSu,FmtTxt)  ' Disabled   TmdX DOF.'
ENDIF

IF ( TmdYDOF )  THEN
   WRITE (UnSu,FmtTxt)  ' Enabled    TmdY DOF.'
ELSE
   WRITE (UnSu,FmtTxt)  ' Disabled   TmdY DOF.'
ENDIF
! Yang: End of adding the output summary of TMD configuration.
...
IF ( ( ADAMSPrep == 1 ) .OR. ( ADAMSPrep == 3 ) )  THEN  ! An ADAMS model will be created; thus, print out all the cols.

...
! Variables in reading
    ...
    real,allocatable :: Disp(:,:),Vel(:,:),Acce(:,:),Force(:,:),TdTensionLT(:),TdTensionUT(:),AddedPtAcce(:,:),MoorTensionFL(:),MoorTensionAC(:)
    real :: SimTime,WvEle
    logical :: wrtFlag
...
! Read the results in loop
      ...
      if (NumAddedPts>0) then
          do j=1,NumAddedPts
              ...
              open (UnFiles(4*NumBodies)+j,file = trim(Outbase)//'_'//'AddedPt_'//trim(adjustl(charAddedPts))//'_Acceleration.dat')
               write(UnFiles(4*NumBodies)+j,'(A)') 'Acclerations of the added point '//trim(adjustl(charAddedPts))//' of the '//trim(Outbase)//' case extracted from AQWA results using the code developed by Y.Yang (PDRA in LJMU)'//trim(Time_Date)
               write(UnFiles(4*NumBodies)+j,'(A10,3(A13))')'Time','x_Acce','y_Acce','z_Acce'
               write(UnFiles(4*NumBodies)+j,'(A10,3(A13))') '(s)','(m/s^2)','(m/s^2)','(m/s^2)'
          enddo
      endif
     ...
     ! Mooring lines    
      open (UnFiles(4*NumBodies)+NumAddedPts+3, file =trim(Outbase)//'_'//'MooringTension.dat')
       write (UnFiles(4*NumBodies)+NumAddedPts+3,'(A)') 'Tension of mooring lines of the '//trim(Outbase)//' case extracted from AQWA results using the code developed by Y.Yang (PDRA in LJMU)'//trim(Time_Date)
       write (UnFiles(4*NumBodies)+NumAddedPts+3,'(A10,A)') 'Time',trim(charMoor)  
       write (UnFiles(4*NumBodies)+NumAddedPts+3,'(A10,A)') 's',trim(charUnit)
       
    ! Wave elevation
      open (UnFiles(4*NumBodies)+NumAddedPts+4, file =trim(Outbase)//'_'//'WaveElevation.dat')
       write (UnFiles(4*NumBodies)+NumAddedPts+4,'(A)') 'Wave elevation '//trim(Outbase)//' case extracted from AQWA results using the code developed by Y.Yang (PDRA in LJMU)'//trim(Time_Date)
       write (UnFiles(4*NumBodies)+NumAddedPts+4,'(A10,A13)') 'Time','WvEle'
       write (UnFiles(4*NumBodies)+NumAddedPts+4,'(A10,A13)')  '(s)','(m)'
...
! Allocate matrix
      ...
      open(1,file = trim(InputLISFile))
       ...
       do    ! time step
          ...
          if (LineStr(1:11) == ' TIME(SECS)') then   ! time step
             ...
             ! CoG results
                ...
                if (NumAddedPts>0) then
                    do j=1,NumAddedPts
                       write(UnFiles(4*NumBodies)+j,'(F10.3,3(ES13.5))')SimTime,(AddedPtAcce(j,k),k=1,3)
                    enddo
                endif
                ...
                do i=1,NumMoorLines
                   ...
                   TempChar = trim(TempChar)//adjustr(CharTension)
                enddo
                write(UnFiles(4*NumBodies)+NumAddedPts+3,'(F10.3,A)') SimTime,trim(TempChar)
                ! Wave elevation
                if (FlagWvElv) then
                   write (UnFiles(4*NumBodies)+NumAddedPts+4,'(F10.3, ES13.5)') SimTime,WvEle
                endif
          ...
          elseif (LineStr(1:33) == '                      TOTAL FORCE') then     ! Total force
       ...
       if (NumAddedPts>0) then
           do j=1,NumAddedPts
              close(UnFiles(4*NumBodies)+j)
           enddo
       endif
       close(UnFiles(4*NumBodies)+NumAddedPts+1)
       close(UnFiles(4*NumBodies)+NumAddedPts+2)
       close(UnFiles(4*NumBodies)+NumAddedPts+3)
       close(UnFiles(4*NumBodies)+NumAddedPts+4)
